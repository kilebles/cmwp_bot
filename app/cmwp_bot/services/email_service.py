...
async def send_discuss_email(
    ...
    html = f"""
    <h3>🗣 Запрос обсуждения проекта</h3>
    <p><b>Имя:</b> {full_name}<br>
    <b>Телефон:</b> {phone or "—"}<br>
    <b>Компания:</b> {company or "—"}<br>
    <b>Профиль:</b> <a href="{username_link}">{username_link}</a></p>
...
async def send_prices_email(full_name: str, username_link: str, phone: str, company: str, answers_text: str):
    msg = EmailMessage()
    msg['From'] = config.SMTP_USER
    msg['To'] = config.EMAIL_ADRESS
    msg['Subject'] = f'Запрос стоимости — {full_name}'
